...
public final class DefaultPluginFileManager implements PluginFileManager {
    ...
    public boolean installPlugin(final String... filenames) {
        ...
        for (String filename : filenames) {
            try {
                FileUtils.moveToDirectory(new File(pluginsTmpPath + getProperty(FILE_SEPARATOR) + filename), new File(
                        pluginsPath), false);
            } catch (IOException e) {
    ...
    public File uploadPlugin(final PluginArtifact pluginArtifact) {
        ...
        File pluginFile = new File(pluginsTmpPath + getProperty(FILE_SEPARATOR) + pluginArtifact.getName());
        try {
    ...
    public void uninstallPlugin(final String... filenames) {
        for (String filename : filenames) {
            File file = new File((pluginsTmpPath + getProperty(FILE_SEPARATOR) + filename));
            if (!file.exists()) {
                file = new File(pluginsPath + getProperty(FILE_SEPARATOR) + filename);
            }
            try {
    ...
    private boolean checkFileExists(final String key, final String path) {
        File file = new File(path + getProperty(FILE_SEPARATOR) + key);
        if (!file.exists()) {
            return false;
        }
        return true;
